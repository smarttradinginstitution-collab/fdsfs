...
async def test_update_trade_recalculates_net_pl(async_client: AsyncClient, db_session: AsyncSession):
    """
    Testa che l'aggiornamento delle commissioni su un trade
    attivi il ricalcolo del P&L netto.
    """
    trading_account_id = await setup_trading_account(async_client, db_session)

    # 1. Crea un trade con P&L lordo e netto iniziale
    trade_payload = {
        "trading_account_id": trading_account_id,
        ...
        "commissions": 2.0,
        "p_l": 93.0,  # 100 - 5 - 2
        "direction": "LONG", # required field
        "status": "closed" # required field
    }
    create_response = await async_client.post("/api/v1/trades/", json=trade_payload)
    assert create_response.status_code == 201
    trade_id = create_response.json()["id"]

    # 2. Aggiorna solo le commissioni
    update_payload = {
            "gross_p_l": 100.0,
            "fees": 5.0,
        "commissions": 10.0
    ...
    # 3. Verifica che il P&L netto sia stato ricalcolato
    # Nuovo P&L netto atteso: 100 (lordo) - 5 (fees) - 10 (nuove commissioni) = 85
    assert updated_trade["p_l"] == pytest.approx(85.0)
    assert updated_trade["commissions"] == pytest.approx(10.0)
    assert updated_trade["gross_p_l"] == pytest.approx(100.0)
    assert updated_trade["fees"] == pytest.approx(5.0)