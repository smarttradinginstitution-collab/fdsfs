...
def image_service(mock_db_session, mock_supabase_client):
    ...
    service.trade_repo = AsyncMock(spec=TradeRepository)
    return service
...
async def test_upload_trade_image_success(image_service: ImageService, mock_supabase_client):
    ...
    mock_trade.trading_account.general_account_id = uuid.uuid4()

    image_service.trade_repo.get_by_id_and_general_account.return_value = mock_trade
    image_service.image_repo.create.return_value = Image(id=uuid.uuid4())

    result = await image_service.upload_trade_image(
        file=mock_file, user_id=user_id, trade_id=trade_id, description="A test image"
    )

    assert result is not None
    image_service.trade_repo.get_by_id_and_general_account.assert_awaited_once()
...
async def test_delete_image_success(image_service: ImageService, mock_supabase_client):
    ...
    mock_image = Image(id=image_id, trade_id=trade_id, storage_path=storage_path)
    image_service.image_repo.get_by_id.return_value = mock_image
    image_service.trade_repo.get_by_id_and_general_account.return_value = Trade(id=trade_id)
