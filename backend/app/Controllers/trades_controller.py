...
def create_trade(
    ...
    """
    return service.create_trade(current_user, trade_data)
...
def get_trades_for_trading_account(
    ...
    """
    return service.list_trades_by_trading_account(current_user, trading_account_id)
...
def get_trade(
    ...
    """
    trade = service.get_trade(current_user, trade_id)
    if not trade:
        raise HTTPException(
...
def update_trade(
    ...
    """
    updated_trade = service.update_trade(current_user, trade_id, trade_data)
...
def delete_trade(
    ...
    Elimina un Trade.
    """
    success = service.delete_trade(current_user, trade_id)
    if not success:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Trade non trovato o non appartenente all'utente.",
