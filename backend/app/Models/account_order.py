...
class AccountOrder(Base):
    ...
    total_quantity: Mapped[float] = mapped_column(Numeric, nullable=True)
    filled_quantity: Mapped[float] = mapped_column(Numeric, nullable=True)
    execution_price: Mapped[float] = mapped_column(Numeric, nullable=True)
    limit_price: Mapped[float] = mapped_column(Numeric, nullable=True)
    time_placed: Mapped[datetime.datetime] = mapped_column(TIMESTAMP(timezone=True), nullable=True)
