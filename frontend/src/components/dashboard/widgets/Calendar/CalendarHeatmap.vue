...
.calendar-container {
  ...
  height: 100%;
  /* The BaseWidget provides padding, so we can remove the old calendar-card padding */
}

.calendar-grid {
  display: grid;
  grid-template-columns: repeat(7, 1fr) auto;
  grid-template-rows: auto; /* Let the header row have natural height */
  grid-auto-rows: minmax(0, 1fr); /* Make content rows expand */
