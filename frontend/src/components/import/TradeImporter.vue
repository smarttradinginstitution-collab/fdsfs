...
const handleUpload = async () => {
  ...
  try {
    ...
    const messageType = response.data.status === 'applied' ? 'success' : 'info';
    const message = response.data.status === 'applied'
      ? 'File already processed. Showing previous result.'
      : 'File uploaded! Processing has started in the background.';
...
<template>
  <div class="trade-importer">
    ...
    <div v-if="importResult" class="import-result">
      ...
      <p><strong>Status:</strong> <span :class="`status-${importResult.status}`">{{ importResult.status }}</span></p>
