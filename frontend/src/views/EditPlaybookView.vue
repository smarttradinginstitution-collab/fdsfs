...
const addNewBlock = async () => {
  const blockTypeInput = prompt("Enter the block type (e.g., CONDITIONS, THESIS, GALLERY):", "CONDITIONS");
  if (!blockTypeInput) return; // User cancelled

  const blockType = blockTypeInput.toUpperCase();
  const validTypes = ["THESIS", "GALLERY", "CONDITIONS", "PSYCHOLOGY", "LEGACY_RULES"];
